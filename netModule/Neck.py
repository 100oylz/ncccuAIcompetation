...
class Neck(nn.Module):
    ...
    def createLayer(self):
        self.up_sample_1 = nn.Upsample(scale_factor=2, mode=neck_upsample_mode)
        # TODO:这里解决顺时针第二个concat的问题，已解决，加了个conv2d
        self.up_sample_2=nn.Sequential(nn.Upsample(scale_factor=2,mode=neck_upsample_mode),
                                       nn.Conv2d(in_channels=self.channel_512_w,out_channels=self.channel_256_w,kernel_size=3,stride=1,padding=1))
        ...
        # TODO:这里是用来解决顺时针第三个concat的问题
        self.conv2d=nn.Conv2d(in_channels=self.channel_512_w,out_channels=self.channel_256_w,kernel_size=3,padding=1,stride=1)
    def forward(self, x1, x2, x3):

        x3_upsample = self.up_sample_1(x3)

        x3_upsample = torch.cat((x2, x3_upsample), dim=1)
        res=self.conv2d(x3_upsample)

        # 第一个CSPLayer计算
        x3_upsample = self.csplayer_2conv_1(x3_upsample)

        x2_upsample = self.up_sample_2(x3_upsample)
        ...
        # TODO:这里有问题
        x2_upsample = torch.cat((res, x3_upsample), dim=1)
